...
if (isset($_POST['action']) && $_POST['action'] == "pass_update"){
	...
	if($op == "" || $np1 == "" || $np2 == ""){
		...
		if($op != $db_pass_str){
        	...
        	$subject = 'TalentTaps Account Password Changed';
        	...
        	$message .= 'possible from your TalentTaps Account page at &nbsp; TalentTaps http://localhost/job/reset.php';
			$message .= '<br /><br /><span style="font-size:16px;"><b>TalentTaps Recovery Team</b></span></body></html>';

			$customMailer->sendMail($db_email, $subject, $message);
