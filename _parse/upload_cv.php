...
if (isset($_FILES["cvfile"]["name"]) && $_FILES["cvfile"]["tmp_name"] != ""){
	...
	$moveResult = move_uploaded_file($fileTmpLoc, "../_USER/$log_email/$fileName");
	if ($moveResult != true) {
		echo "File upload failed";
		exit();
	}
	$sql2 = "UPDATE seeker_profile SET cv='$fileName' WHERE e_hash='$log_email'";
	$query2 = mysqli_query($db_connection, $sql2);
	mysqli_close($db_connection);
	$returnfile = "";
	if($moveResult) {
		$targetDisplay = "_USER/$log_email/$fileName";
		$returnfile .= '<h3><b style="color:crimson;font-size: 16px;">'.$fileName.'</b></h3>';
